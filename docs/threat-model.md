...
| E | **Bypass ngưỡng quorum qua "phantom bit" trong `participantBitmap`** (`LightClientVerifierBLS.sol`, `LightClientVerifierBLSGeneral(RFC9380).sol`, Milestone 4/5) — `_popcount` đếm TẤT CẢ bit trong bitmap (uint256 256 bit / bytes theo byte đầy đủ), trong khi `_aggregatePubkeys` chỉ duyệt `[0, numValidators)`. Khi `numValidators` không phải bội số của 8 (bản `bytes`) hoặc luôn (bản `uint256` cố định 5 validator, 251 bit "ma" không dùng), kẻ tấn công set thêm bit ở vị trí không tương ứng validator nào thật để đếm popcount đủ `threshold`, trong khi aggregate pubkey/chữ ký chỉ cần khớp với SỐ VALIDATOR THẬT ít hơn nhiều — bỏ qua yêu cầu đa số 2/3 thật sự. Phát hiện qua self-review bảo mật có chủ đích (`/security-review`) trên code mới Milestone 5, không phải tự nhiên tìm ra khi viết. | ✅ Đã sửa — `_popcount` giới hạn đúng `[0, numValidators)` (khớp `_aggregatePubkeys`), thêm `_checkPaddingBitsZero`/kiểm tra `bitmap >> NUM_VALIDATORS != 0` từ chối thẳng bitmap có bit "ma" thay vì âm thầm bỏ qua. Test khai thác thật: `test_updateState_rejectsPhantomBitPadding` (cả 2 contract) — ký thật bởi 1 validator, set thêm bit ma để popcount giả đạt threshold, xác nhận bị revert `PaddingBitsMustBeZero` đúng chỗ; kèm test đối chứng xác nhận trường hợp thiếu quorum trung thực (không có bit ma) vẫn báo đúng `InsufficientQuorum`, không bị fix làm hỏng hành vi cũ. 65/65 test pass sau khi vá (thêm 3 test so với trước). |
...
| 2 | `CHALLENGE_PERIOD` (1 ngày) đủ để bên bị hại (hoặc watchtower thay mặt) phát hiện & phản ứng | Bên bị hại offline lâu hơn 1 ngày VÀ không có watchtower nào theo dõi kênh → mất tiền vào state cũ | ✅ Đã có watchtower (Milestone 4, `watchtower/`) — xem `watchtower/README.md`. Watchtower là bên thứ 3 tự động gọi `challenge()` thay mặt party offline, dùng checkpoint (state + 2 chữ ký) đã tự verify trước đó. Cần đổi contract: bỏ `onlyParty` khỏi `challenge()`/`challengeWithProof()` (an toàn không đổi — vẫn cần 2 chữ ký hợp lệ/ZK proof, chỉ nới lỏng AI được gửi tx). **Giới hạn còn lại**: chỉ bảo vệ được state đã thực sự checkpoint (nếu party bỏ qua 1 vòng update không gửi checkpoint, watchtower không biết để bảo vệ). ~~1 watchtower là 1 điểm liveness duy nhất~~ — **✅ Đã verify chạy nhiều instance độc lập thật**: `watchtower/src/multi_watchtower_e2e.ts` (`pnpm run e2e:multi`) chạy **3 watchtower thật** (HTTP server thật + event listener thật qua `startMonitoring` — lần đầu tiên trong repo test đường HTTP checkpoint API + event-listener thật đầu-cuối, mọi demo trước chỉ gọi hàm trực tiếp), tắt hẳn 2/3 (đóng server, không gắn listener) TRƯỚC khi partyA gian lận — watchtower còn sống sót duy nhất vẫn tự phát hiện + cứu đúng channel, payout cuối khớp chính xác. Kiến trúc vốn đã hỗ trợ (không cần sửa code) vì `challenge()` không giới hạn số lượng bên gọi. |
...
| 7 | Relayer trung thực trong việc chuyển tiếp state/proof | Relayer ác ý có thể trì hoãn/từ chối chuyển tiếp (liveness), nhưng KHÔNG thể giả mạo proof hay attest sai giá trị (an toàn/safety vẫn giữ nhờ ZK + quorum check trong `LightClientVerifier.updateState`) — cần phân biệt rõ 2 loại rủi ro này khi trình bày demo. ✅ **Đã có watch-loop** (Milestone 4, `relayer/src/watch.ts`, `pnpm run watch`) — subscribe `ChannelClosedCooperatively`/`ChannelClosedUnilaterally`/`ChannelChallenged` trên chain nguồn, tự relay state mới nhất sang chain đích ngay khi có event, không cần gọi tay `index.ts` sau mỗi update. Đã verify thật: mở kênh + `closeUnilateral` trên Chain A, watch-loop tự phát hiện và relay sang Chain B mà không có thao tác thủ công nào. ~~1 relayer instance vẫn là 1 điểm liveness duy nhất~~ — **✅ Đã verify chạy nhiều instance độc lập thật**: `relayer/src/multi_relayer_e2e.ts` (`pnpm run e2e:multi`) chạy **3 relayer thật** (`attachWatchQueue` tách ra từ `watch.ts` để gắn nhiều lần), tắt 2/3 (gỡ event listener) TRƯỚC khi channel đổi state — relayer sống sót duy nhất vẫn tự relay đúng sang chain đích. **Lỗi thật tìm được khi verify**: `watch.ts` thiếu guard `require.main === module` (khác mọi entry-point khác trong repo) — `import` module này để lấy `attachWatchQueue` vô tình CHẠY LUÔN `main()` của chính nó, gây 2 relayer trùng đụng nonce (`replacement transaction underpriced`). Đã vá bằng đúng guard chuẩn đã dùng ở nơi khác. Các relay đồng thời (nếu có) được serialize qua 1 queue trong `watch.ts` để tránh race nonce ví relayer. |
...
## Known issue phát hiện khi test trên fork mạng thật (Sepolia)

Deploy thử `watchtower/src/e2e_demo.ts` lên fork Sepolia thật
(`anvil --fork-url <RPC Sepolia thật>`, không phải Anvil local thuần) —
điều chưa bao giờ làm suốt các milestone trước, chỉ chạy trên Anvil local.
Phát hiện: **balance đọc qua `provider.getBalance()` ngay sau `tx.wait()`
có thể trả về giá trị CŨ** (từ trước khi tx vừa confirm được áp dụng), dù
`tx.wait()` đã trả về receipt thành công — tái lập ổn định nhiều lần, kể cả
dùng "fresh provider instance" (workaround đã có sẵn trong code cho vấn đề
tương tự trên Anvil local) và dù chờ 1.5s vẫn xảy ra ở 1 số lần đọc. Diễn
giải qua debug trực tiếp (`eth_getBalance` bằng `curl`, so sánh trước/sau
từng bước): đây là **race giữa xác nhận tx và khả năng RPC trả đúng state
mới**, không phải lỗi hợp đồng — tiền vẫn đúng vị trí on-chain, chỉ là
script off-chain (relayer/watchtower) đọc nhầm số dư tại đúng thời điểm đó.
Rủi ro thật nếu không sửa: 1 watchtower/relayer thật chạy trên mạng thật có
thể ra quyết định sai dựa trên state đọc nhầm (VD tưởng chưa `withdraw()`
xong nên gọi lại, hoặc log sai số liệu).

**✅ Đã sửa** — `watchtower/src/rpcSync.ts` + `relayer/src/rpcSync.ts` (2 bản
giống nhau, mỗi package tự chứa theo convention sẵn có): `waitForBlockAtLeast`
(chờ RPC báo đã bắt kịp block mục tiêu) + `readUntilStable` (đọc lặp lại tới
khi 2 lần đọc liên tiếp khớp nhau) gộp thành `readConfirmed`. Áp dụng vào 2
điểm đọc state quan trọng nhất trước khi ra quyết định: `watchtower/src/monitor.ts`
(`reactToChannel` đọc `channels()` trước khi quyết định `challenge()`, nhận
`minBlockNumber` từ chính block của event kích hoạt) và `relayer/src/watch.ts`
(`RelayQueue.relayOne` đọc `channels()` trước khi relay, cùng cơ chế).

**Lỗi thứ 2 phát hiện được trong lúc điều tra** (khác hẳn, không liên quan
staleness): chạy watchtower e2e trên Anvil local (không fork) để kiểm chứng
fix, phát hiện `challenge()` fail **tái lập ổn định 100%** với `OutOfGas`
thật (xác nhận bằng `cast run <txhash>` — calldata trên chain đầy đủ, hợp
lệ, không phải lỗi encode phía client). Nguyên nhân: `estimateGas()` của
ethers ước tính thấp hơn ~3.7% so với gas thực tế cần khi tx thực thi. **Đã
sửa** — thêm `sendWithGasBuffer` (buffer 15% trên gas estimate) trong cùng
`rpcSync.ts`, áp dụng cho `challenge()` (watchtower) và `updateState()`
(relayer) — 2 điểm gửi tx quan trọng nhất. Verify thật: watchtower e2e chạy
**3/3 lần liên tiếp thành công** sau fix (trước đó fail 100%, kể cả baseline
chưa có fix balance-staleness); relayer e2e cross-chain + watch-loop tự động
đều verify lại thành công với đường code mới.
