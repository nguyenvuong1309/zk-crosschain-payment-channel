...
async function main() {
  ...
  const partyAOnA = new ethers.Wallet(PARTY_A_KEY, providerA);
  const partyBOnA = new ethers.Wallet(PARTY_B_KEY, providerA);
  const partyAOnB = new ethers.Wallet(PARTY_A_KEY, providerB);
  const partyBOnB = new ethers.Wallet(PARTY_B_KEY, providerB);

  const paymentChannelA = new ethers.Contract(deployment.chainA.paymentChannel, paymentChannelAbi, providerA);
  const paymentChannelB = new ethers.Contract(deployment.chainB.paymentChannel, paymentChannelAbi, providerB);

  console.error("--- Step 1: open + join a channel on Chain A ---");
  let tx = await paymentChannelA.connect(partyAOnA).open(partyBOnA.address, DEPOSIT_A, 0, 0, 0, 0, 0, { value: DEPOSIT_A });
  ...
  tx = await paymentChannelB.connect(partyAOnB).open(partyBOnB.address, DEPOSIT_A, 0, 0, 0, 0, 0, { value: DEPOSIT_A });
  ...
  tx = await paymentChannelB
    .connect(partyAOnB)
    .closeWithRemoteAttestation(channelIdB, deployment.chainA.paymentChannel, deployment.chainA.chainId, state);
  await tx.wait();
  ...
  const balABefore = await providerB.getBalance(partyAOnB.address);
  const balBBefore = await providerB.getBalance(partyBOnB.address);
  ...
  const balAAfter = await providerB.getBalance(partyAOnB.address);
  const balBAfter = await providerB.getBalance(partyBOnB.address);
